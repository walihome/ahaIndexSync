...
class DeepMindBlogScraper(BaseScraper):
    ...
    def fetch(self) -> list[RawItem]:
        try:
            res = requests.get(self.BLOG_URL, headers={"User-Agent": "Mozilla/5.0"}, timeout=15)
            ...
            for card in soup.select("a[href*='/blog/']"):
                href = card.get("href", "")
                if not href or href in ("/discover/blog/", "/blog/") or href.count("/") < 3:
                    continue
                if href in seen:
                    continue
                seen.add(href)
